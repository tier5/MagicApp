        ...
        <v-form>
          ...
          <v-layout row wrap>
            ...
            <v-flex
              md4
              xs12
              v-if="!!urlParams.validationType && (urlParams.validationType === '=' || urlParams.validationType === '!=') "
            >
              <v-card-text>
                <v-text-field v-model="urlParams.field_value" placeholder="Value"></v-text-field>
              </v-card-text>
            </v-flex>
              ...
              <v-card-text>
                <ul>
                  <li v-for="(obj, index) in newZap.params" :key="index">
                    ...
                    <img
                      src="../../../assets/images/cross_btn.png"
                      alt="icon"
                      class="icon-cross"
          ...
          <v-layout row wrap>
            ...
            <v-flex
              md6
              xs12
              v-bind:class="{ 'form-error-box': $v.htmlIdentifier.attribute_name.$error }"
            >
              <v-card-text>
                <v-text-field
                ...
                >Id Name is not allowed!</span>
              </v-card-text>
            </v-flex>
            <v-flex
              md5
              xs12
              v-bind:class="{ 'form-error-box': $v.htmlIdentifier.attribute_type.$error }"
            >
              <v-card-text>
                <v-select
                  ...
                  @blur="$v.htmlIdentifier.attribute_type.$touch"
                ></v-select>
                <span>ex.name</span>
                <span
                  class="validation-error-message"
              ...
              <v-card-text>
                <ul>
                  <li v-for="(obj, index) in newZap.element_attributes" :key="index">
                    <span>{{obj.attribute_name}} &rarrtl; {{obj.attribute_type}}</span>
                    <img
                      src="../../../assets/images/cross_btn.png"
                      alt="icon"
                      class="icon-cross"
...
export default {
  ...
  computed: {
    ...mapGetters(["isSuccess", "isError" , "user"]),
    timeoutProgess: function() {
      if (
        (this.newZap.timeout.days ||
          this.newZap.timeout.minutes ||
          this.newZap.timeout.hours) &&
        !this.$v.newZap.timeout.$invalid
      ) {
        return true;
      } else {
        return false;
